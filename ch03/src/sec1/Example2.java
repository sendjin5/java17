package sec1;

public class Example2 {
    public static void main(String[] args) {
        //if~else
        int js1 = 80, js2 = 90, js3 = 95;
        float py = (js1 + js2 + js3) / 3.0f;
        //평균이 80점 이상이면, 합격, 아니면 탈락


        /*it~else~
        if(조건식){조건이 참일 때 실행할 문장;}
         else

        */


        if (py >= 80) {
            System.out.println("합격");
        } else {
            System.out.println("탈락");
        }
        if (js1 >= 80 && js2 >= 80 && js3 >= 80) System.out.println("장학생 대상");
        else System.out.println("장학금 대상자 아님");
        if (js1 >= 90 || js2 >= 90 || js3 >= 90) System.out.println("과목 우수자");
        else System.out.println("과목 우수 아님");


        //if~else if else~
        /*
        if(조건식1)
            {조건식1이 만족할 경우 실행할 문장}
        else if(조건식2)
            {조건식1이 만족하지 않고, 조건식2가 만족할 경우 실행할 문장}
        ...반복
          */





        if (py >= 90) {
            System.out.println("수");
        } else if (py >= 80) {              //=if(py<90 && py>=80)
            System.out.println("우");
        } else if (py >= 70) {
            System.out.println("미");
        } else if (py >= 60) {
            System.out.println("양");
        } else {
            System.out.println("가");
        }



        /*switch~case~break 문
        switch(기준식 또는 변수) {
            case 값1:
                기준식 또는 변수의 결과값이 값1과 일치할 경우 실행할 문장
                break
            case 값n:
                기준식 또는 변수의 결과값이 값n과 일치할 경우 실행할 문장
                break
             default:
                어떠한 case에도 속하지 않는 경우 실행할 문장
             ※기준값 또는 변수의 결과값만큼 case의 값과 경우의 수가 같아야하며,
               같이 않은 경우 default 를 설정하여라, 또한 break를 걸지 않으면 나머지 case가 모두 연산됨
         */

        int pt = (int)py / 10;
        String grade = "";
        switch (pt) {
            case 1:
            case 2:
            case 3:
            case 4:
            case 5:
                grade = "5등급";
                break;
            case 6:
                grade = "4등급";
                break;
            case 7:
                grade = "3등급";
                break;
            case 8:
                grade = "2등급";
                break;
            case 9:
                grade = "1등급";
                break;
            default: grade = "판정불가";
        }
        System.out.println(grade);




    }
}